...
Required permissions for the SAS token are: Read, Add, Create, Write, List

Note that the env var `SAS_TOKEN` is not required if `REMOTE_PATH` includes the sas token (i.e. is a full sas url).
